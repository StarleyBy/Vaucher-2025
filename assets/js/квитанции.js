document.addEventListener('DOMContentLoaded', async () => {
    ...
    const studentsData = await getSheetData('MAIN!A:S', accessToken);
    if (studentsData) {
    ...
    receiptsForm.addEventListener('submit', async (e) => {
        ...
        let receipt5000Link = '';
        if (receipt5000File) {
            const uploadResponse = await uploadFile(receipt5000File, RECEIPT_5000_FOLDER_ID, accessToken);
            if (uploadResponse) {
        ...
        let receipt200Link = '';
        if (receipt200File) {
            const uploadResponse = await uploadFile(receipt200File, RECEIPT_200_FOLDER_ID, accessToken);
            if (uploadResponse) {
                receipt200Link = uploadResponse.webViewLink;
            }
        }

        const response = await updateSheetData(`MAIN!R${rowNum}:S${rowNum}`, [[receipt5000Link, receipt200Link]], accessToken);
